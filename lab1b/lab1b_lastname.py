...
def zero(x, y, level=0, debug=False):
    ...
    """
    z = y - level          # thus find the 0 crossing in z
    n = len(x)             # y and x should have the same length
    if len(x) != len(y):   # y and x should have the same length
        print("ZERO: input array are not the same length")
    if debug: print(f"ZERO: array has {n} elements. First and last value {z[0]} {z[-1]}")   
    s0 = np.sign(z[0])
    for i in range(1,n):
        if np.sign(z[i]) != s0:
    ...
    if debug: print("ZERO: never found a crossing point")
    return 0.0
